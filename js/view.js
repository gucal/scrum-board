(function () {
  ...
  View.prototype.render = function (page, args) {
    if (page === 'Home') {
      document.querySelector('content').innerHTML = this.templates.Home(
        args.boards,
      );
    } else if (page === 'Board') {
      let scrolled =
      ...
      document.querySelector('content').innerHTML = this.templates.Board(
        args.board,
      );
  ...
  View.prototype.setInputError = function (inputID) {
    this.querySelector('#' + inputID).className += this.getElement(
      '#' + inputID,
  ...
  View.prototype.showHelp = function () {
    this.showModal(
      'Help',
      `You can edit a board name or a list name by clicking on it. <br>
