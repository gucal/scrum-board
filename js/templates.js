(function () {
  ...
  function Board({ id, title, lists }) {
    return `<header class="header shadow">
                ...
                <main class="padding row">
                    ...
                    <div>
                        <button id="add-list" board="${
                          this.id
                        }" class="icon-button"><i class="lnr lnr-plus-circle"></i></button>
                    </div>
                </main>`;
  }
  ...
  function List({ id, title, items }) {
    return `<div class="list bg-white shadow border-radius">
                    ...
                    <div>
                    <p>Bu listenin tahmini tamamlanma süresi: <b>${
                      items.length * 5
                    }</b> saattir.</p>
                    </div>
                </div>`;
  ...
  function Modal(title, text, hasButtons) {
    return `<div class="modal-container">
                    ...
                    <div class="modal border-radius shadow bg-white">
                        ...
                        ${
                          hasButtons
                            ? `<button id="no-button" class="link">No</button><button id="yes-button" class="link">Yes</button>`
                            : ''
                        }
                        </div>
                    </div>
