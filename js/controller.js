(function () {
  ...
  Controller.prototype.setHomeEvents = function () {
    ...
    this.view.getElements('.remove-board').forEach((button) => {
      this.view.addEvent(button.id, 'click', () =>
        this.removeBoard(button.id.split('-')[2]),
      );
    });
    this.view.getElements('.board-button').forEach((button) => {
      this.view.addEvent(button.id, 'click', () =>
        this.openBoard(button.id.split('-')[2]),
      );
    });
  };
  ...
  Controller.prototype.setBoardEvents = function () {
    ...
    this.view.getElements('.remove-list').forEach((button) => {
      this.view.addEvent(button.id, 'click', () =>
        this.removeList(button.id.split('-')[2]),
      );
    });
  ...
  Controller.prototype.setTitleEditionEvents = function (
    textID,
    inputID,
    type,
  ) {
  ...
  Controller.prototype.removeBoard = function (boardID) {
    this.view.showModal(
      'Panoyu sil?',
      'Panoyu silmek istediğinizden emin misiniz?',
      true,
      () => {
    ...
    if (value === '') {
      if (type !== 'item') {
        this.view.setInputError(
          type == 'board' ? 'board-title-input' : 'list-' + id + '-title-input',
        );
      } else {
        this.removeItem(id);
      }
  ...
  Controller.prototype.dragEnter = function (event) {
    if (
      ...
      !isPreview(event.fromElement) &&
      !isSelected(event.currentTarget) &&
      !previousIsSelected(event.currentTarget)
    ) {
  ...
  Controller.prototype.targetPosition = function (
    event,
    sourceItemID,
    targetListID,
  ) {
